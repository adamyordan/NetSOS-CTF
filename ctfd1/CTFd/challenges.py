...
from flask import render_template, request, redirect, jsonify, url_for, session, Blueprint, current_app as app
...
def api_chals():
    if request.form.get('supersecret') != app.config['NETSOS_SUPER_SECRET']:
        return jsonify(ok=False, error="'supersecret' not specified or incorrect")
