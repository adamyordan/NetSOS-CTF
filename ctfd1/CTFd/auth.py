...
@auth.route('/' + app.config['ADMIN_SECRET_URL'] + '/confirm', methods=['POST', 'GET'])
@auth.route('/' + app.config['ADMIN_SECRET_URL'] + '/confirm/<data>', methods=['GET'])
...
@auth.route('/' + app.config['ADMIN_SECRET_URL'] + '/reset_password', methods=['POST', 'GET'])
@auth.route('/' + app.config['ADMIN_SECRET_URL'] + '/reset_password/<data>', methods=['POST', 'GET'])
...
@auth.route('/' + app.config['ADMIN_SECRET_URL'] + '/register', methods=['POST', 'GET'])
def admin_register():
    if not can_register():
        return redirect(url_for('auth.login'))
    if request.method == 'POST':
        ...
        if len(errors) > 0:
            return render_template('admin_register.html', errors=errors, name=request.form['name'], email=request.form['email'], password=request.form['password'])
        else:
            with app.app_context():
                team = Teams(name, email.lower(), password, -1)
                db.session.add(team)
                db.session.commit()
                db.session.flush()
...
@auth.route('/' + app.config['ADMIN_SECRET_URL'] + '/login', methods=['POST', 'GET'])
def admin_login():
    if request.method == 'POST':
        errors = []
        name = request.form['name']
...
def api_register():
    if not can_register():
        return errorJson('registration not allowed');

    if not request.form.get('name'): return errorJson("'name' not specified")
    if not request.form.get('email'): return errorJson("'email' not specified")
    ...
    with app.app_context():
        team = Teams(name, email.lower(), password, int(portal_id))
        team.verified = True
        db.session.add(team)
        db.session.commit()
        db.session.flush()
...
def api_login():
    ...
    if not request.form.get('name') or not request.form.get('password'):
        return errorJson('params not specified')
...
@auth.route('/api/session', methods=['POST'])
def api_session():
    if authed():
        sessDict = dict((name, session[name]) for name in session 
            if not name.startswith('__') and name != 'nonce')
        return jsonify(ok=True, auth=True, session=sessDict)
